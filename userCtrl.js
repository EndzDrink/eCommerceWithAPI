...
const loginUserCrtl = asyncHandler(async(req, res ) => {
    ...
    //checking if the user exist or non
...
const handleRefreshToken = asyncHandler(async (req, res) => {
    ...
    if(!user)
    throw new Error('No Refresh Token present in the dataase or token is not matched');
...
const forgotPasswordToken = asyncHandler(async (req, res) => {
    ...
    try{
        ...
        await user.save();
        const resetURL = `Hello, Please follow this link to reset Your Password. This link is valid for 10 minutes from now. <a href='http://localhost:5000/api/user/reset-password/${token}>Click Here</a>`
        const data = {
            to: email,
            text: "Hello user",
...
const applyCoupon = asyncHandler(async(req, res) =>{
    ...
    let totalAfterDiscound = (cartTotal - (cartTotal * validCoupon.discount)/100)
    .toFixed(2);
